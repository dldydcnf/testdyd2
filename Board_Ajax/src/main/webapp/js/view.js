...
function getList(state){//현재 선택한 댓글 정렬방식을 저장합니다. 1=>등록순, 2=>최신순
      ...
      $.ajax({
         ...
         success:function(rdata){
            $('#count').text(rdata.listcount).css('font-family','arial,anas-serif')   
            // 선택한 문장의 갯수를 가져오는 쿼리문?
            let red1='red';
            let red2='red';
            if(state==1){
               red2='gray';
          ...
          if(rdata.boardlist.length>0){ //등록순 과 최신순이 나오는경우는 boardlist.length가 0보다 클경우만
