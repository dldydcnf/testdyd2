...
<body>
	...
	<form name="joinform" action="updateProcess.net" method="post"
		...
		<h3>회원 정보 수정</h3>
		<hr>
		<b>아이디</b> <input type="text" name="id" value="${memberinfo.id }"
			readonly> 
		<b>비밀번호</b> <input type="password" name="pass"
			value="${memberinfo.password }" readonly> 
		<b>이름</b> <input
			type="text" name="name" value="${memberinfo.name }"
			placeholder="Enter name" required> 
		<b>나이</b> <input
			type="text" name="age" value="${memberinfo.age }" maxlength="2"
		...
		<b>이메일 주소</b> <input type="text" name="email"
			...
			id="email_message"></span> <b>프로필 사진</b> <label> <img
			src="image/attach.png" width="10px"> <span id="filename">${memberinfo.memberfile }</span>
			<span id="showImage"> <c:if
					test='${empty memberinfo.memberfile }'>
		...
		</span> <%-- accept 속성은 사용자가 파일 선택 대화 상자를 열 때 허용되는 파일의 타입을 지정하는 데 사용됩니다. 
          "image/*" 값은 모든 이미지 파일을 허용하도록 지정합니다.
          
          <input type="file" accept="파일 확장자|audio/*|video/*|image/*">
             (1) 파일 확장자는 .png , .jpg , .pdf , .hwp 처럼 (.)으로 시작하는 파일 확장자를 의미합니다.
    ...
    $("input[name=email]").on('keyup',
          function(){
             ...
             const email_value=$("input:eq(6)").val();
    ...
    $('input[type=file]').change(function(event){
    	...
    	if(pattern.test(filename)){
    		...
    		const reader = new FileReader(); //파일을 읽이 위한 객체 생성
