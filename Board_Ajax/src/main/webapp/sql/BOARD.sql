...
--2. board와 조인을 합니다.
/*
SELECT comment_board_num, board_subject, count(*) cnt
FROM comm c
INNER JOIN board b ON c.comment_board_num = b.board_num
GROUP BY c.comment_board_num, b.board_subject;
*/
...
from board
full outer join (select comment_board_num,count(*) cnt
				from comm
				group by comment_board_num)
*/
select board_num, board_subject, nvl(cnt,0) as cnt
...
from ( select rownum rnum, j.*
		from (select board_num, board_subject, nvl(cnt,0) as cnt
			 from board left outer join (select comment_board_num, count(*) cnt
			 from comm
			 group by comment_board_num)
on board_num = comment_board_num
