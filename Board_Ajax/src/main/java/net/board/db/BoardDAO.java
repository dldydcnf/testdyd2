...
public class BoardDAO {
	...
	public List<BoardBean> getBoardList(int page, int limit) {
		...
		String board_list_sql = "SELECT * " + " FROM (SELECT ROWNUM RNUM, J.* "
				...
				+ "   on board_num=comment_board_num" + "   ORDER BY BOARD_RE_REF DESC," + "   BOARD_RE_SEQ ASC) J "
	...
	public boolean boardInsert(BoardBean boarddata) {
		...
		// 원문글의 BOARD_RE_REF 필드는 자신의 긃번호 입니다.
		...
		try (Connection con = ds.getConnection(); PreparedStatement pstmt = con.prepareStatement(sql);) {
			...
			// 원문의 경우 BOARD_RE_LEV, BOARD_RE_SEQ 필드값은 0입니다.
	...
	public int boardReply(BoardBean board) {
		 ...
		 * 됩니다. 글 목록에서 보여질 때 하나의 그룹으로 묶여서 출력됩니다.
		 */
	...
	private void reply_update(Connection con, int re_ref, int re_seq) throws Exception {
		...
		// 현재 글을 이미 달린 답글보다 앞에 출력되게 하기 위해서입니다.
